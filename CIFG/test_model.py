...
char_to_idx = {'l': 0, '\n': 1, 'k': 2, 's': 3, 'o': 4, 'c': 5, ';': 6, '-': 7, 'd': 8, 'y': 9, "'": 10, ')': 11, 'j': 12, 'u': 13, 'z': 14, 'i': 15, 'n': 16, 'b': 17, 'v': 18, ',': 19, 'p': 20, 'h': 21, 'q': 22, 'r': 23, 'e': 24, 'x': 25, '.': 26, 'g': 27, 't': 28, '(': 29, 'w': 30, 'f': 31, ':': 32, '?': 33, 'a': 34, '!': 35, 'm': 36, ' ': 37}
...
class unitRead(unittest.TestCase):
    def setUp(self) -> None:
        self.basics = LSTM(char_to_idx = char_to_idx, idx_to_char = idx_to_char, vocab_size = 38)
    def test_forward(self) -> None:
            ...
            out, v, h, o, c, c_bar, i, f, z = self.basics.forward_step(test_x, test_h_p, test_c_p)
    ...
    def test_tanh(self) -> None:
            ...
            if np.allclose(self.basics.tanh(test_vec), np.tanh(test_vec)) and np.allclose(self.basics.tanh(test_vec_2), np.tanh(test_vec_2)) :
                same = True
    ...
    def test_softmax(self) -> None:
            ...
            if np.allclose(self.basics.softmax(test_vec), np.array([0.0021657, 0.00588697, 0.11824302, 0.87370431])):
                same = True
            self.assertEqual(same, True, "Test failed for tanh")
    def test_backward(self):
            ...
            # Mock shape items
            dh_next = dc_next = c_prev = f = i = c_bar = c = i = h = o = np.random.randn(100, 1)
            test_z = np.random.randn(y_hat.shape[0] + dh_next.shape[0], 1)

            dh_prev, dc_prev = self.basics.backward_step(3, y_hat, dh_next, dc_next, c_prev, test_z, f, i, c_bar, c, o, h)
            self.assertEqual(dc_prev.shape, dc_next.shape, "Test backpropagation failed for dc_prev")
            self.assertEqual(dh_prev.shape, dh_next.shape, "Test backpropagation failed for dh_next")           
